...
{
  tiredness = get_attribute(olympicsGames,"tiredness")
  reset = get_attribute(olympicsGames,"reset")
  u1 <- runif(1,0,1)
  u2 <- runif(1,0,1)
  
  if(reset==1 && (now(olympicsGames)>320)){ 
    tiredness=0  #reset tiredness
    ...
    if(2/3< u1 & u1<=1)
    {tiredness <-  (1-(1/9-u2/9)^(1/2))}
    
    return(c(tiredness,0))
  }
  else{
    
    if(u1 <= 1/3)
    {AddToTiredness <-  (u2/8)^(1/3) +tiredness}
    
    if(1/3< u1 & u1<=2/3)
    {AddToTiredness <- (u2+3)/6 +tiredness}
    
    if(2/3< u1 & u1<=1)
    {AddToTiredness <-  (1-(1/9-u2/9)^(1/2)) +tiredness}
    
    return(c(AddToTiredness,reset)) 
  }
}
